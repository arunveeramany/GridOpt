...
int PowerNet::readGAMS(const string& fname) {
    ...
    while (word.compare("/;")) {
        file >> word;
        auto gen = gens[idx++];
        gen->_pbound.min = atof(word.c_str());
        file >> word;
    }
    ...
    while (word.compare("/;")) {
        file >> word;
        auto gen = gens[idx++];
        gen->_pbound.max = atof(word.c_str());
        file >> word;
    }
    ...
    while (word.compare("/;")) {
        file >> word;
        auto gen = gens[idx++];
        gen->_qbound.min = atof(word.c_str());
        file >> word;
    }
    ...
    while (word.compare("/;")) {
        file >> word;
        auto gen = gens[idx++];
        gen->_qbound.max = atof(word.c_str());
        file >> word;
    }
    ...
    for (auto gen:gens) {
        gen->print();
        name = gen->_name;
        qg_max.set_val(name,gen->_qbound.max/bMVA);
        qg_min.set_val(name,gen->_qbound.min/bMVA);
        pg_max.set_val(name,gen->_pbound.max/bMVA);
        pg_min.set_val(name,gen->_pbound.min/bMVA);
...
void PowerNet::save_base_case_sol(const string& fname){
    ...
    unsigned idx = 0;
    for (auto gen:gens) {
        myfile << gen->_bus->_name << "," << gen->_unit_name << "," << pg_vals->at(idx)*bMVA << "," << qg_vals->at(idx)*bMVA << endl;
        idx++;
    }
...
void PowerNet::save_all_sol(const string& fname){
    ...
    for (cont = 0; cont<_conting_lines.size(); cont++) {
        ...
        idx = 0;
        for (auto gen:gens) {
            myfile << cont+1 << "," << gen->_name<< "," << gen->_bus->_name << "," << gen->_unit_name << "," << qg_vals->at(idx)*bMVA << endl;
            idx++;
        }
    }
    auto g_idx = 0;
    ...
    for (auto node:nodes) {
        myfile << "0," << node->_name << "," << v_vals->at(idx) << "," << theta_vals->at(idx)*180/pi << endl;
        idx++;
    }
    for (auto cont = 0; cont<_nb_conting; cont++) {
        ...
        idx = 0;
        for (auto node:nodes) {
            myfile << cont+1 << "," << node->_name << "," << v_vals->at(idx) << "," << theta_vals->at(idx)*180/pi << endl;
            idx++;
        }
    }
    ...
    for (auto cont = 0; cont<_nb_conting; cont++) {
        myfile << cont+1 << ","<< _delta[cont].eval() << endl;
    }
    ...
    idx = 0;
    for (auto arc:arcs) {
        myfile << "0," << arc->_name << "," << arc->_circuit_id << "," << p_from_vals->at(idx)*bMVA << "," << q_from_vals->at(idx)*bMVA << "," << p_to_vals->at(idx)*bMVA << "," << q_to_vals->at(idx)*bMVA << endl;
        idx++;
    }
    cont = 0;
    /** Line Contingencies **/
    for (cont = 0; cont<_conting_lines.size(); cont++) {
        ...
        for (auto arc:arcs) {
            ...
            myfile << cont << "," << arc->_name << "," << arc->_circuit_id << "," << p_from_vals->at(idx) << "," << q_from_vals->at(idx)<< "," << p_to_vals->at(idx) << "," << q_to_vals->at(idx) << endl;
            idx++;
        }
    }
    /** Gen Contingencies **/
...
unique_ptr<Model> PowerNet::build_ACOPF_N_1(PowerModelType pmt, int output, double tol){
    ...
    for (auto i = 0; i<nb_lines_cont; i++) {
        ...
        ACOPF->_obj += 1e6*sum(v_diff_p) + 1e6*sum(v_diff_n);
